...
- **4.25** · 2026-09-25 · `58d0d3b` — Stop TV-client accounts mining games nobody asked for. 4.24 let a campaign of unknown link state be tried so inventory could report whether it was linked, which in practice meant a fresh account mining whatever ended soonest (Albion Online, Black Desert, Escape from Tarkov: Arena). Such a campaign is now held to the same rule as an unlinked one — mined only for a prioritised game — unless inventory already shows the account linked for that game, since Twitch links a game account rather than a campaign. Inventory's drop definitions now replace the ones read from a live channel, which omit reward types: WARDOGS' badge had been mined with badge campaigns switched off. The all-games browse reads four orders in parallel, cutting a TV account's first campaign pass from about 56 seconds. Miner cards and status follow the drop actually being earned rather than a subscription reward listed ahead of it.
- **4.24** · 2026-09-25 · `6e1be3a` — Bring accounts signed in with Twitch's TV client close to how new accounts behaved before September's sign-in change. They now find campaigns across every game with live drops-enabled streams, not only their prioritised games: Browse → Live Channels filtered to drops is read from both ends of the viewer count (one scan shared by every account for 15 minutes), and each game's campaigns are read from one of its channels, or three for prioritised games. Twitch will not tell a TV account whether a campaign's game account is linked until the campaign has progress, so a found campaign is now treated as unknown rather than unlinked: selection may try it after every linked or prioritised candidate, its first credited minute puts it in inventory with the real link state, and a campaign that turns out to be unlinked drops back out under the usual rule. Unknown-link campaigns no longer raise "link your account" warnings.
- **4.23** · 2026-09-25 · `bc0c133` — Restore adding accounts after Twitch stopped accepting the Android app's client ID for new device sign-ins (`invalid client`, 2026-09-18). New accounts, including Discord invitations, now sign in with Twitch's TV client, and every account keeps the client its token was issued to, so accounts added earlier stay on Android untouched. TV-issued tokens can watch, read inventory and drop progress, and claim, but Twitch returns an empty drops dashboard to them, so a TV account finds campaigns the way a viewer does: live drops-enabled channels for each prioritised game and the campaigns those channels are running, with inventory filling in allow-lists and account-link state once watching starts. TV accounts therefore discover new campaigns only for prioritised games.
- **4.22** · 2026-09-23 · `8a05cf2` — Stop reopening the app's window from marking every miner "Blocked — Needs attention" while they kept earning. Reopening a backgrounded app re-ran launch auto-start against miners that were already mining; the engine refused the second start without logging it, and that refusal was recorded as a failed start. Starting a miner that is already running is now a logged no-op, which also covers a Resume from SwiftBot or the web dashboard. Also keeps a campaign benched for repeated no-progress stalls out of the same-game fallback list for its 30-minute cooldown: it was being re-added as a fallback at once, so two miners rewatched a dead PUBG campaign on the same channel every two minutes, logging a "left the candidate set" warning each time.
- **4.21** · 2026-09-21 · `7910732` — Stop a live but ineligible approved channel from waking a waiting miner into a full campaign and inventory scan every minute. Idle probes now confirm the candidate campaign is active on the channel, automatic claim scans keep their two-minute cadence across unrelated rescans, and unchanged restricted-channel PubSub subscriptions remain installed between scans instead of being torn down and rebuilt.
- **4.20** · 2026-09-15 · `e89c428` — Keep unattended-health uptime tied to the miners in the current launch. SwiftMiner now removes live snapshots left by earlier launches or deleted accounts while preserving system health and resolved incident history, which restores the Overview's healthy-duration reading and stops the health file growing forever. Health records and cleanup share one non-blocking ordered queue, so a delayed cleanup from an earlier account addition cannot erase a newer miner's snapshot.
- **4.19** · 2026-09-14 · `819cbe2` — Stop reporting a miner as not earning the moment it starts watching after a quiet stretch. The earning clock ran from the later of the last banked progress and the worker start, so a miner that earned, then spent hours with no eligible campaign or no live channel, carried those hours into its next stream: one opened a "not earning for 925 minutes" incident one second after watching began, and the incident history showed the same pattern repeatedly — both miners flagged together within seconds of watching starting, cleared about a minute later by the first progress, never notified. The manager now stamps when a miner leaves a status with nothing to watch (idle, no eligible campaigns, waiting for a stream, paused, blocked on an account link), and the clock runs from the latest of that, the last progress and the worker start. The routine watching → fetching → watching churn never moves it, so a miner genuinely watching without earning is still flagged at 20 minutes.
- **4.18** · 2026-09-11 · `ea63c02` — Keep campaign discovery working after Twitch retired the persisted query behind its Drops dashboard, and recover when GraphQL reports an upstream timeout inside an otherwise successful HTTP response. The dashboard query now uses Twitch's replacement hash; service timeouts, cancelled upstream requests, unavailable service responses, and exceeded context deadlines retry through the existing bounded backoff instead of being mistaken for a changed response shape and stopping that mining cycle.
- **4.17** · 2026-09-11 · `d31121f` — Stop three things a 16-hour diagnostic showed the engine doing to no purpose. Miners that re-picked a channel in the same instant all read the same occupancy snapshot — which only counts channels a miner has already *started* watching — so each avoided the one taken stream and all landed on the same free one, defeating the stream spreading the setting had switched on; the manager now picks and reserves in a single uninterrupted step, so a choice is visible to the next miner before it has been acted on. Every successful claim was followed by a notification-delete call that has never once succeeded: its persisted query hash is stale, and it was passing a drop-instance ID where a notification ID belongs, so 50 claims meant 50 failures and about five wasted seconds each inside the claim loop — the call is gone. And the filter tally that explains a scheduling decision was being emitted by call sites that only wanted the campaign list to refresh the UI, which put 1,144 duplicate lines in that same log, neighbouring entries disagreeing about how many campaigns were eligible because each read a different snapshot; only the scheduler's own call logs now. Two Activity Log changes come with it: the PubSub viewer-count line is throttled to one per channel per five minutes instead of one per ten seconds (it was 17% of the log, and it is kept at all because a silently dead PubSub is what the log is read to find), and an HTTP error body is condensed to one line, so Twitch's 15-line Varnish outage page no longer lands in full.
- **4.16** · 2026-09-08 · `3a1f9a3` — Stop miners watching channels that have already stopped broadcasting. Twitch's game directory keeps listing a stream for a while after it ends, and the per-channel campaign check that follows it confirms the campaign is running there rather than that anyone is live, so a miner could re-select the very channel whose stream-down it had just handled — one diagnostic caught it four seconds after the event, then heartbeating that dark channel for 2h13m while Spade accepted every beacon. A stream-down is now recorded in the fleet-wide liveness cache the moment it arrives, and directory results are screened against it, so the sighting one miner pays for spares the other four. As a backstop for the ending nobody saw, a watch session that has gone five minutes without server-verified progress re-checks its channel directly and switches if it is dark; a crediting stream reports progress about once a minute, so a healthy session never reaches that check and never spends the request.
- **4.15** · 2026-09-07 · `2639bf1` — Let anti-stall recovery run on campaigns whose reward is offered at several watch-time tiers, and stop two failure modes from being invisible. A benefit ID shared across tiers (Rainbow Six's "Esports Pack" at 60, 180 and 360 minutes) made every stall window read the unclaimed tier as claimed somewhere else, reset the stall counter, and keep the miner on a channel crediting nothing — three accounts logged over three hours of watching for zero minutes earned. External claims are now read from the inventory merge's own verdict, so recovery switches channel or cools the campaign down as intended. Stall windows are tagged so the Activity Log keeps the alarm and the decision it led to together instead of evicting the decision within a day. A miner idle with nothing eligible now publishes when it will next look for work, which the miner card counts down and diagnostic exports record — it used to sit silent between scans and read as stuck. And when Twitch refuses to serve follow state, channel ranking still carries on without it, but now says so in the Activity Log instead of leaving followed-streamer prioritisation switched on and quietly inert.
