...
// MARK: - Persistence comparisons

extension Progress {
    /// `lastUpdated` is observation metadata and is regenerated by otherwise-identical
    /// Twitch responses. Excluding it prevents timestamp-only disk rewrites.
    func isPersistenceEquivalent(to other: Progress) -> Bool {
        id == other.id
            && dropId == other.dropId
            && dropName == other.dropName
            && campaignId == other.campaignId
            && currentMinutes == other.currentMinutes
            && requiredMinutes == other.requiredMinutes
            && isClaimed == other.isClaimed
    }
}

extension Drop {
    func isPersistenceEquivalent(to other: Drop) -> Bool {
        id == other.id
            && name == other.name
            ...
            && preconditionDrops == other.preconditionDrops
            && requiredSubs == other.requiredSubs
            && dropStartDate == other.dropStartDate
            && dropEndDate == other.dropEndDate
            && Self.progress(progress, equals: other.progress)
    }

    private static func progress(_ lhs: Progress?, equals rhs: Progress?) -> Bool {
        switch (lhs, rhs) {
        case (.none, .none): true
        case let (.some(lhs), .some(rhs)): lhs.isPersistenceEquivalent(to: rhs)
        default: false
        }
    }
}
...
extension Campaign {
    func isPersistenceEquivalent(to other: Campaign) -> Bool {
        id == other.id
            && name == other.name
            && game == other.game
            && status == other.status
            ...
            && isAccountConnected == other.isAccountConnected
            && allowIsEnabled == other.allowIsEnabled
            && isPrioritised == other.isPrioritised
            && drops.persistenceElementsEqual(other.drops)
    }
}

extension Array where Element == Campaign {
    func persistenceElementsEqual(_ other: [Campaign]) -> Bool {
        guard count == other.count else { return false }
        let lhs = sorted { $0.id < $1.id }
        let rhs = other.sorted { $0.id < $1.id }
        return zip(lhs, rhs).allSatisfy { $0.isPersistenceEquivalent(to: $1) }
    }
}

extension Array where Element == Drop {
    func persistenceElementsEqual(_ other: [Drop]) -> Bool {
        guard count == other.count else { return false }
        let lhs = sorted { $0.id < $1.id }
        let rhs = other.sorted { $0.id < $1.id }
        return zip(lhs, rhs).allSatisfy { $0.isPersistenceEquivalent(to: $1) }
...
extension Array where Element == Progress {
    func persistenceElementsEqual(_ other: [Progress]) -> Bool {
        guard count == other.count else { return false }
        let lhs = sorted { $0.id < $1.id }
        let rhs = other.sorted { $0.id < $1.id }
        return zip(lhs, rhs).allSatisfy { $0.isPersistenceEquivalent(to: $1) }
