...
///   presentation).
///
/// The minor counts engine-changing commits within the era, so `4.21` is the
/// twenty-first engine change since that decomposition.
///
...
public enum MinerEngineVersion {
    ...
    /// mining behaviour.
    public static let current = "4.22"

    /// The date `current` last moved, as `yyyy-MM-dd`.
    public static let updated = "2026-09-23"
